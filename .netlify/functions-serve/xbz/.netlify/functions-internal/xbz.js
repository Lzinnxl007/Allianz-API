...
var require_src = __commonJS({
  "node_modules/@fast-csv/format/build/src/index.js"(exports2) {
    ...
    var write = (rows, options) => {
      const csvStream = (0, exports2.format)(options);
    ...
    var writeToPath = (path, rows, options) => {
      const stream = fs3.createWriteStream(path, { encoding: "utf8" });
...
var require_src2 = __commonJS({
  "node_modules/@fast-csv/parse/build/src/index.js"(exports2) {
    ...
    var parseFile = (location, options = {}) => {
      return fs3.createReadStream(location).pipe(new CsvParserStream_1.CsvParserStream(new ParserOptions_1.ParserOptions(options)));
    };
...
// .netlify/functions-internal/xbz.js
var xbz_exports = {};
__export(xbz_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(xbz_exports);

// src/modules/CSV/CSV.service.js
var import_fast_csv = __toESM(require_src3(), 1);
var import_node_fs = __toESM(require("fs"), 1);

// src/modules/XBZ/XBZ.service.js
var XBZService = class _XBZService {
  static async manipulateProducts(products) {
    const XBZAllProducts = await _XBZService.#getXbzProducts();
    const XBZCodes = products.map((product) => product.codigo);
    const XBZCategories = products.map((product) => product.categoria);
    const filteredXBZProducts = _XBZService.#filterXBZProducts(
      XBZAllProducts,
      XBZCodes
    );
    const productsWithCategories = _XBZService.#addCategory(
      filteredXBZProducts,
      XBZCategories
    );
    const updatedProducts = _XBZService.#addProductType(productsWithCategories);
...
var XBZ_service_default = XBZService;

// src/modules/XBZ/XBZ.controller.js
var import_node_fs2 = __toESM(require("fs"), 1);
var XBZController = class {
  static async ManipulateProducts() {
    const file = import_node_fs2.default.readFileSync("XBZData.json", "utf-8", (error) => {
      if (error) {
        console.log(error);
        return null;
      }
    ...
    const data = await XBZ_service_default.manipulateProducts(products);
    return data ?? [];
  }
};
var XBZ_controller_default = XBZController;

// .netlify/functions-internal/xbz.js
var handler = async (event, context) => {
  const data = await XBZ_controller_default.ManipulateProducts();
  return {
    statusCode: 200,
